...
const Login = ({ setUser }) => {
  ...
  const handleLogin = (e) => {
    ...
    const mockUser = {
      name: 'John Doe',
      email: email,
      role: email.includes('recruiter') ? 'RECRUITER' : (email.includes('admin') ? 'ADMIN' : 'APPLICANT')
    };
    setUser(mockUser);
    localStorage.setItem('user', JSON.stringify(mockUser));
    
    if (mockUser.role === 'RECRUITER') navigate('/recruiter');
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-8 md:p-10">
        ...
        <div className="mt-8 text-center text-sm text-gray-500 font-medium">
          Don't have an account? <Link to="/signup" className="text-indigo-600 hover:underline">Create an account</Link>
        </div>
      </div>
    </div>
  );
